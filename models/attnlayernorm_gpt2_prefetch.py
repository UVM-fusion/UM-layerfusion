...
class AttentionWithLayerNorm(nn.Module):
    ...
    def forward(self, x):
        # Apply LayerNorm directly before Attention
        ln_x = nn.LayerNorm(x.size(-1)).cuda()(x)  # LayerNorm applied to the input tensor
        
        # Now apply Attention mechanism to the normalized tensor
        query, key, value = self.c_attn(ln_x).split(self.split_size, dim=2)
...
class Block(nn.Module):
    def __init__(self, prefetcher, embed_dim, num_heads, num_positions, pdrop=0.1):
        ...
        self.attn = AttentionWithLayerNorm(prefetcher, embed_dim, num_positions, num_heads, pdrop=pdrop).cuda()

        # MLP layers and LayerNorm
    ...
    def forward(self, x):
        ...
        # Attention with LayerNorm fusion
        hidden_states = hidden_states + self.attn(hidden_states)

        # MLP layer with LayerNorm
